...
def test_replaced_serializer():
    ...
    class DetailViewSet(viewsets.ViewSet):
        ...
        @swagger_auto_schema(
            responses={
                404: openapi.Response("Not found or Not accessible", DetailSerializer)
            }
        )
...
@pytest.mark.skipif(
    not MethodMapper or not action,
    reason="action.mapping test (djangorestframework>=3.9 required)",
)
def test_action_mapping():
    ...
    for _ in range(3):
        ...
        assert all(
            action_ops[mth]["operationId"] == "mapping_" + mth for mth in methods
        )
...
def test_choice_field(choices, expected_type):
    ...
    assert property_schema == openapi.Schema(
        title="Detail", type=expected_type, enum=choices
    )


@pytest.mark.parametrize(
    "choices, field, expected_type",
    [
...
@pytest.mark.skipif(
    typing is None or sys.version_info.major < 3, reason="typing not supported"
)
def test_optional_return_type(py_type, expected_type):
    ...
    router = routers.DefaultRouter()
    router.register(
        r"optional", OptionalMethodViewSet, **_basename_or_base_name("optional")
    )
    ...
    assert property_schema == openapi.Schema(
        title="X", type=expected_type, readOnly=True
    )


EXPECTED_DESCRIPTION = """\
  description: |-
    This is a demo project for the [drf_yasg2](https://github.com/joellefkowitz/drf-yasg) Django Rest Framework library.
