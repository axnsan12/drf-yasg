...
class ArticleSerializer(serializers.ModelSerializer):
    ...
    uuid = serializers.UUIDField(
        help_text="should articles have UUIDs?", read_only=True
    )
    cover_name = serializers.FileField(use_url=False, source="cover", required=True)
    group = serializers.SlugRelatedField(
        slug_field="uuid", queryset=ArticleGroup.objects.all()
    )
...
class ImageUploadSerializer(serializers.Serializer):
    ...
    what_am_i_doing = serializers.RegexField(
        regex=r"^69$", help_text="test", default="69", allow_null=True
    )
