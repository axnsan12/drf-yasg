    ...
    class SchemaView(APIView):
        ...
        def get(self, request, version='', format=None):
            ...
            if isinstance(request.accepted_renderer, _SpecRenderer):
                generator = self.generator_class(info, version, url, patterns, urlconf, swagger_settings)
            else:
                generator = self.generator_class(info, version, url, patterns=[], swagger_settings=swagger_settings)
