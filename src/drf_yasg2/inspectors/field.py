...
class InlineSerializerInspector(SerializerInspector):
    ...
    def field_to_swagger_object(
        ...
        description = getattr(
            field, 'description', getattr(field, '__doc__', None)
        )
        ...
        if isinstance(field, (serializers.ListSerializer, serializers.ListField)):
            ...
            if (
                actual_serializer and actual_serializer != this_serializer
            ):  # pragma: no cover
...
def get_queryset_from_view(view, serializer=None):
    ...
    try:
        ...
        if queryset is not None and serializer is not None:
            ...
            assert type(serializer) == call_view_method(
...
class RelatedFieldInspector(FieldInspector):
    ...
    def field_to_swagger_object(
        ...
        if isinstance(field, serializers.ManyRelatedField):
            ...
            return SwaggerType(
                type=openapi.TYPE_ARRAY, items=child_schema, unique_items=True,
            )
...
if typing:
    ...
    def inspect_collection_hint_class(hint_class):
        ...
        return OrderedDict(
            [("type", openapi.TYPE_ARRAY), ("items", openapi.Items(**child_type_info)),]
        )
...
def get_basic_type_info_from_hint(hint_class):
    ...
    for check_class, info in hinting_type_info:
        if hint_class_issubclass(hint_class, check_class):
            ...
            return OrderedDict([("type", swagger_type), ("format", format),])
    ...
    class RecursiveFieldInspector(FieldInspector):
        ...
        def field_to_swagger_object(
            ...
            if (
                ...
                ), "Can't create RecursiveField schema for inline " + str(type(proxied))
